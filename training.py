...
class Trainer:
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch_data in tqdm(train_loader):
                ...
                if self.log_steps and global_step % self.log_steps == 0:
                    self.logger.info(
                        f"[Train] epoch:{epoch + 1}/{self.n_epochs}, step: {global_step}/{num_training_steps}, step_loss:{loss.item():.4f}")
                ...
                if val_loader is not None and self.eval_steps > 0 and global_step != 0 and \
                    ...
                    self.logger.info(
                        f"[Evaluate] epoch:{epoch + 1}/{self.n_epochs}, step: {global_step}/{num_training_steps}, val_acc:{val_acc:.5f}")
                    ...
                    if val_acc >= self.best_score:
                        self.save_model()
                        self.logger.info(
                            f"[Evaluate] best accuracy performance has been updated: {self.best_score:.5f} -> {val_acc:.5f}")
            ...
            if val_loader is not None:
                ...
                # wandb.log({
                #     "Epoch": epoch+1,
                #     f"train_epoch_loss": f"{tr_loss / tr_examples:.4f}",
                #     "Evaluate_epoch_val_acc": f"{val_acc:.4f}",
                # })
    ...
    def predict(self, data_loader):
        self.model.load_state_dict(torch.load(
            f'{args.save_path}/trained_model.pkl'))
        self.model.eval()
        y_list = []
        y_hat_list = []
        for batch in tqdm(data_loader):
